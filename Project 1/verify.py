...
if __name__ == '__main__':
    ...
    # #setup and check for verify
    # pk = ecdsa.decode_pk(pub_key,128)
    # (r,s) = ecdsa.decode_sig(sigs[0], 128)
    # ecdsa_verify(msgs[0], pk, (r, s)
    ...
    for i in range(6):
        for j in range(6):
            sig = ecdsa.decode_sig(sigs[j])
            if ecdsa_verify(msgs[i], pk, sig) == True:
                print(msgs[i] + " " + sigs[j])
                print("\n")
